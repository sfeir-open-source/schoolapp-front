...
import {
  School,
  schoolConverter,
} from '@schoolApp/shared/interfaces/schools.interface';
...
export const getAllSchools = (status?: StatusType[]) => {
  if (!status?.length) {
    return getDocs(
      collection(firestore, 'schools').withConverter(schoolConverter)
    );
  }
  return getDocs(
    query(
...
export const getSchoolByUid = (uid: string) => {
  return getDoc(
    doc(firestore, `schools/${uid}`).withConverter(schoolConverter)
  );
};
